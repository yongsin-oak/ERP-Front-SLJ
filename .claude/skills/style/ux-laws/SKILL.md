...
- Confirm before delete → `DeleteConfirmButton` / `ConfirmDrawer` (no `Modal.confirm` API post-migration)
...
```tsx
// Inline/table delete — confirm popover baked in
<DeleteConfirmButton
  title="ลบสินค้า?"
  description={`ลบ "${record.name}" ออกถาวร กู้คืนไม่ได้`}
...
```tsx
<Button className="min-h-11 min-w-11" aria-label="ดำเนินการ" />
```
