...
When a feature has more than just the list query — a dropdown/infinite query (`keys.dropdown(search)`), a ref lookup (`[...keys.all, 'ref', …]`) — an **update** mutation must invalidate `keys.all` (prefix-covers every one of them), not just `keys.lists()` — otherwise edited labels go stale in `SearchSelect` dropdowns and in scanned rows.

This is not tidiness — the dropdown is a **different endpoint** (`/<entity>/dropdown-search`)
returning a **different shape** (`CursorPage<T>`, no `pagination`), so its cache entry is
genuinely separate from the table's and nothing about invalidating `keys.lists()` reaches it.

```ts
// queryKeys.ts — dropdown ต้องมี factory ของตัวเอง ห้ามเขียน array ดิบใน hook
...
### Infinite query = cursor เสมอ

`useInfiniteQuery` ในโปรเจกต์นี้มีที่เดียวคือ dropdown และทุกตัวเป็น cursor:
`getNextPageParam: (last) => last.nextCursor ?? undefined` + `initialPageParam: undefined as string | undefined`
(`undefined` = หน้าแรก) — **ห้าม** `pageParam + 1` หรืออ่าน `last.pagination.hasNextPage`
ใน infinite query อีก นั่นคือรูปแบบเก่าที่ย้ายออกไปแล้ว
ดู [`query-constants`](../query-constants/SKILL.md#dropdown--ตาราง--คนละเส้น-คนละ-contract)
