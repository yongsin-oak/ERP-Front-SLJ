...
export interface ShopSearchSelectProps {
  value?: string;
...
export function ShopSearchSelect({
  ...
  const { data, isFetching, isFetchingNextPage, hasNextPage, fetchNextPage } = useShopDropdown(
    search || undefined,
    { enabled },
  );
  ...
  return (
    <div className={cn('relative inline-flex w-full', className)} style={style}>
      ...
      {showClear && (
        <button
          type="button"
          aria-label="ล้างร้านค้าที่เลือก"
          onClick={() => combo.clear()}
