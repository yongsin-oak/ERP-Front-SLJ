...
export function useCreateOrder() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      ...
      notify.success('สร้าง Order สำเร็จ');
    },
...
export function useUpdateOrder() {
  ...
  return useMutation({
    ...
    onSuccess: (updated) => {
      ...
      notify.success('แก้ไข Order สำเร็จ');
    },
...
export function useDeleteOrder() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: orderKeys.lists() });
      notify.success('ลบ Order สำเร็จ');
    },
  ...
  return useMutation({
    ...
    onSuccess: (result, ids) => {
      ...
      if (failed > 0) {
        ...
        notify.success('ลบ Order สำเร็จ', `${deleted} รายการถูกลบออกจากระบบ`);
      }
    },
    onError: handleError('ลบ Order'),
  });
