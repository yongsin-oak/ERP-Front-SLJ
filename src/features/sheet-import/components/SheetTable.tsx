import { useRef } from 'react';
import { useVirtualizer } from '@tanstack/react-virtual';
import { Tooltip } from 'radix-ui';
import { cn } from '@/lib/utils';
import { TOOLTIP_CONTENT } from '@/lib/styles';
import type { SheetData } from '../types';

const ROW_HEIGHT = 38;
const COL_WIDTH = 150;
const ROW_NUM_WIDTH = 52;
const DEFAULT_MAX_HEIGHT = 480;

export interface SheetTableProps {
  data: SheetData;
  /** ดัชนีแถว (เริ่มที่ 0) → รายการข้อความผิดพลาดของแถวนั้น */
  errors?: Record<number, string[]>;
  maxHeight?: number;
}

export function SheetTable({ data, errors = {}, maxHeight = DEFAULT_MAX_HEIGHT }: SheetTableProps) {
  const containerRef = useRef<HTMLDivElement>(null);
  const { headers, rows } = data;
  const minWidth = ROW_NUM_WIDTH + headers.length * COL_WIDTH;

  // ไฟล์นำเข้ามีได้เป็นหมื่นแถว — render จริงเฉพาะแถวที่มองเห็น
  const rowVirtualizer = useVirtualizer({
    count: rows.length,
    getScrollElement: () => containerRef.current,
    estimateSize: () => ROW_HEIGHT,
    overscan: 8,
  });

  return (
    <Tooltip.Provider delayDuration={200}>
      <div
        ref={containerRef}
        className="relative overflow-auto rounded-md border border-border bg-background"
        style={{ height: maxHeight }}
      >
        {/* หัวตารางติดบน */}
        <div
          className="sticky top-0 z-2 flex border-b border-border bg-surface-100"
          style={{ minWidth, height: ROW_HEIGHT }}
        >
          <div
            className="flex h-full shrink-0 items-center justify-end border-r border-border px-2 text-xs font-medium text-foreground-muted"
            style={{ width: ROW_NUM_WIDTH }}
          >
            #
          </div>
          {headers.map((h) => (
            <div
              key={h}
              title={h}
              className="flex h-full shrink-0 items-center truncate border-r border-border-muted px-3 text-xs font-medium text-foreground-lighter"
              style={{ width: COL_WIDTH }}
            >
              {h}
            </div>
          ))}
        </div>

        {/* แถวเสมือน */}
        <div className="relative" style={{ minWidth, height: rowVirtualizer.getTotalSize() }}>
          {rowVirtualizer.getVirtualItems().map((vRow) => {
            const row = rows[vRow.index];
            const rowErrors = errors[vRow.index];
            const hasError = rowErrors !== undefined;

            return (
              <div
                key={vRow.key}
                className={cn(
                  'absolute top-0 left-0 flex border-b border-border-muted',
                  hasError ?
                    'bg-error-bg hover:bg-error-border/30'
                  : 'bg-background hover:bg-accent-overlay',
                )}
                style={{ minWidth, height: ROW_HEIGHT, transform: `translateY(${vRow.start}px)` }}
              >
                <div
                  className={cn(
                    'flex h-full shrink-0 items-center justify-end border-r border-border px-2 text-xs',
                    hasError ? 'text-error-text' : 'text-foreground-muted',
                  )}
                  style={{ width: ROW_NUM_WIDTH }}
                >
                  {hasError ?
                    <Tooltip.Root>
                      <Tooltip.Trigger asChild>
                        <button
                          type="button"
                          aria-label={`ข้อผิดพลาดแถวที่ ${vRow.index + 1}`}
                          className="cursor-help text-xs text-error-text underline decoration-dotted underline-offset-2"
                        >
                          {vRow.index + 1}
                        </button>
                      </Tooltip.Trigger>
                      <Tooltip.Portal>
                        <Tooltip.Content
                          side="right"
                          sideOffset={6}
                          className={cn(TOOLTIP_CONTENT, 'max-w-70')}
                        >
                          {rowErrors.join(' · ')}
                        </Tooltip.Content>
                      </Tooltip.Portal>
                    </Tooltip.Root>
                  : vRow.index + 1}
                </div>
                {headers.map((h) => {
                  const cellVal = String(row[h] ?? '');
                  return (
                    <div
                      key={h}
                      title={cellVal}
                      className="flex h-full shrink-0 items-center truncate border-r border-border-muted px-3 text-sm text-foreground"
                      style={{ width: COL_WIDTH }}
                    >
                      {cellVal}
                    </div>
                  );
                })}
              </div>
            );
          })}
        </div>
      </div>
    </Tooltip.Provider>
  );
}
